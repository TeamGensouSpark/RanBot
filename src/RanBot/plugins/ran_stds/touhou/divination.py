...
async def handle(event: MessageEvent):
    ...
    await divination.finish(
        Message([MessageSegment.at(event.get_user_id()), random.choice(choices)])
    )


divination_intro = on_command(
    "divination_intro",
    rule=to_me(),
    aliases={
        "占卜介绍",
...
async def handle(event: MessageEvent):
    await divination_intro.finish(
        Message([MessageSegment.at(event.get_user_id()), introduce])
    )
